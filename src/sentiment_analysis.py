...
entity_property_pair = [
    '제품 전체#품질', '제품 전체#편의성', '제품 전체#디자인', '제품 전체#일반', '제품 전체#가격','제품 전체#인지도','제품 전체#다양성', 
...
def parse_args():
    ...
    parser.add_argument(
        "--train_target", type=str, default="Entity",
        help="train target"
    )
    parser.add_argument(
...
def train_sentiment_analysis(args=None):
    ...
    for _ in trange(epochs, desc="Epoch"):
        ...
        if args.train_target== 'Entity':
            ...
            for step, batch in tqdm(enumerate(entity_property_train_dataloader), total=len(entity_property_train_dataloader)):
            ...
            for step, batch in tqdm(enumerate(polarity_train_dataloader), total=len(polarity_train_dataloader)):
                batch = tuple(t.to(device) for t in batch)
                b_input_ids, b_input_mask, b_labels = batch
...
def predict_from_korean_form(tokenizer, ce_model, pc_model, data):
    ...
    for sentence in data:
        ...
        # if sentence['annotation'] is None:
...
def test_sentiment_analysis(args):
    ...
    # 아니 test에는 annotation이 업성서 f1을 구할수가 없어 시발 ㅋㅋ; 이거 테스트하려면 따로 train뽑아야될듯

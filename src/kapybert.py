...
def parse_args():
    ...
    parser.add_argument(
        "--base_model", type=str, default="bert-base-multilingual-cased"
    )
    parser.add_argument(
...
def train_sentiment_analysis(args=None):
    ...
    print('loading model')
    entity_property_model = BertBaseClassifier(args, len(label_id_to_name), len(tokenizer))
    entity_property_model.to(device)

    polarity_model = BertBaseClassifier(args, len(polarity_id_to_name), len(tokenizer))
...
def test_sentiment_analysis(args):
    ...
    model = BertBaseClassifier(args, len(label_id_to_name), len(tokenizer))
